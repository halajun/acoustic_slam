        ...
        for (int x = 0; x < u1.nx; x++)
        {
            ...
            for (int trial = 0; trial < maxtrials; trial++)
            {
                ...
                if (hypot(tmpoff[0], tmpoff[1]) < minoff ||
                    !check_inside_image(tmp[0], tmp[1], u2))
                    continue;
    ...
    for (int j = 0; j < u1.ny; j++)
    {
        ...
        for (int i = 0; i < u1.nx; i++)
        {
            ...
            for (int trial = 0; trial < 4; trial++)
            {
                ...
                if (!check_inside_image(neigh[0], neigh[1], u1))
                    continue;

                PointD2 noff(off(neigh[0], neigh[1], 0), off(neigh[0], neigh[1], 1));

                if (noff[0] == 0 || noff[1] == 0)
                    continue;
...
int main(int argc, char **argv)
{
    ...
    // --- read input data paths --- //
    {
        ...
        options.add_options()("help", "Print help")("image", "Input folder containing sss image files", cxxopts::value(strImageFolder))("pose", "Input folder containing auv pose files", cxxopts::value(strPoseFolder))("altitude", "Input folder containing auv altitude files", cxxopts::value(strAltitudeFolder))("groundrange", "Input folder containing ground range files", cxxopts::value(strGroundRangeFolder))("annotation", "Input folder containing annotation files", cxxopts::value(strAnnotationFolder));
    ...
    for (size_t i = 0; i < test_num; i++)
    {
        ...
        Frame tmp_frame = Frame(i, vmImgs[i], vmPoses[i], vvAltts[i], vvGranges[i], vmAnnos[i]);
        test_frames.push_back(tmp_frame);
        cout << "complete!" << endl;
    ...
    for (size_t i = 0; i < test_frames.size(); i++) // test_frames.size()
    {
        cv::Mat new_anno_kps;
        ...
        for (size_t j = 0; j < test_frames.size(); j++)
        {
            ...
            for (size_t n = 0; n < test_frames[i].anno_kps.rows; n = n + 1)
            {
                ...
                {
                    cout << "not in between image " << test_frames[i].img_id << " and " << test_frames[j].img_id << "..." << endl;
                    continue;
                }
                ...
                {
                    count2++;
                    cout << "(1) save non-overlapping matches..." << endl;
                    new_anno_kps.push_back(test_frames[i].anno_kps.row(n));
                    continue;
                }
                ...
                {
                    count++;
                    cout << "(2) checking questionable matches... " << sqrt(e_rc_tmp) << endl;
                    ...
                    cout << "Waiting for youuuur call..." << endl;
                    bool save_or_not;
                    std::cin >> save_or_not;
                    if (save_or_not)
                    {
                        cout << "you decide to SAVE the matches!" << endl;
                        new_anno_kps.push_back(test_frames[i].anno_kps.row(n));
                        count1++;
                    }
                    else
                    {
                        cout << "you decide to DISCARD the matches!" << endl;
                        count0++;
                    }
                }
                else
                {
                    count3++;
                    cout << "(0) saving consistent matches..." << endl;
                    new_anno_kps.push_back(test_frames[i].anno_kps.row(n));
                }
            ...
            // cout << "Metric Error between " << test_frames[i].img_id << " and " << test_frames[j].img_id << " (r c rc %): ";
            // cout << e_r << " " << e_c << " " << e_rc << " " << (float)count / test_num * 100 << "%"
            //      << " (" << count << "/" << test_num << ")" << endl;
            cout << "Non-overlapping/questionable/consistent number: " << count2 << " " << count << "(" << count0 << "/" << count1 <<  ")" << " " << count3 << endl;

        }

        // save new annotated keypoints
        stringstream ss;
        ss << setfill('0') << setw(4) << test_frames[i].img_id+170;
        string path= "SSH-" + ss.str() + ".xml";
        cv::FileStorage fs(path, cv::FileStorage::WRITE);
        fs << "new_annos_kps" << new_anno_kps;
        fs.release();

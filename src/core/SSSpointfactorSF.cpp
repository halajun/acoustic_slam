...
gtsam::Vector SssPointFactorSF::evaluateError(const gtsam::Point3& p, const gtsam::Pose3& T,
                                            boost::optional<gtsam::Matrix&> H1,
                                            boost::optional<gtsam::Matrix&> H2) const {

    gtsam::Point3 p_s = Ts_.transformTo( Tp_.transformTo( T.transformTo(p) ) );

    // note that use boost optional like a pointer
    // only calculate jacobian matrix when non-null pointer exists
    if (H1)
    {
        gtsam::Rot3 J_s_kp = Ts_.rotation().inverse()*Tp_.rotation().inverse()*T.rotation().inverse();
    ...
    if (H2)
    {
        ...
        if (plan_a)
        {
            block_t = -(Ts_.rotation().inverse()*Tp_.rotation().inverse()*T.rotation().inverse()).matrix();
            gtsam::Point3 p_m = T.transformTo(p);
            gtsam::Matrix3 m_x = (gtsam::Matrix3() << 0.0, -p_m.z(), p_m.y(),
                                                       p_m.z(), 0.0, -p_m.x(),
                                                       -p_m.y(), p_m.x(), 0.0).finished();
            gtsam::Matrix3 block_r = Ts_.rotation().inverse().matrix()*Tp_.rotation().inverse().matrix()*m_x;
        ...
        else
        {
            ...
            block_t = -(Ts_.rotation().inverse()*Tp_.rotation().inverse()*T.rotation().inverse()).matrix();
            gtsam::Vector3 col_4 = (Ts_.rotation().inverse()*Tp_.rotation().inverse()*rot_dev_r).rotate(p-T.translation());
            gtsam::Vector3 col_5 = (Ts_.rotation().inverse()*Tp_.rotation().inverse()*rot_dev_p).rotate(p-T.translation());
            gtsam::Vector3 col_6 = (Ts_.rotation().inverse()*Tp_.rotation().inverse()*rot_dev_y).rotate(p-T.translation());

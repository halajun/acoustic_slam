...
cv::Mat Frame::GetFilteredMask(const cv::Mat &sss_raw_img)
{
    ...
    for (size_t i = 0; i < output_mask.rows; i++)
    {
        for (size_t j = 0; j < output_mask.cols; j++)
        {
            ...
            if (sss_raw_img.at<double>(i,j)>MeanofMat[0]*factor)
            {
                for (size_t x = i-r; x < i+r; x++)
                {
                    ...
                    {
                        if (x<output_mask.rows && y<output_mask.cols)
                        {
            ...
            if (j<side*factor2 || j>(output_mask.cols-side*factor2))
                output_mask.at<bool>(i,j) = 0;
        }
                                      ...
                                      const std::vector<double> &tf_stb, const std::vector<double> &tf_port)
{
    ...
    // cv::Mat bin_loc_x(cv::Size(col, row), CV_64FC1, Scalar(0)); 
    // cv::Mat bin_loc_y(cv::Size(col, row), CV_64FC1, Scalar(0)); 
